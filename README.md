...
The API is organized into different sections:
...
- `POST /auth/check-code` - Verify activation code
- `POST /auth/resend-email` - Resend activation email
...
- `POST /admin/users/profile` - Get user profile by ID (Admin only)
- `GET /admin/users/:id` - Get user by ID (Admin only)
- `PATCH /admin/users/profile` - Update user profile by ID (Admin only)
- `PATCH /admin/users` - Update any user (Admin only)
...
- **Admin**: Requires valid JWT token with admin privileges
